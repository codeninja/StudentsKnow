module Technoweenie # :nodoc:
  module AttachmentFu # :nodoc:
    @@default_processors = %w(Rffmpeg ImageScience Rmagick MiniMagick )
    @@tempfile_path      = File.join(RAILS_ROOT, 'tmp', 'attachment_fu')
    @@content_types      = ['image/jpeg', 'image/pjpeg', 'image/gif', 'image/png', 'image/x-png', 'image/jpg']
    @@video_types      = ['video/mp4', 'video/flv', 'video/x-flv', 'video/mpg', 'video/mpeg', 'video/mp2', 'video/wmv', 'video/x-mpeg', 'video/quicktime', 'video/x-ms-asf', 'video/x-msvideo', 'video/vnd.objectvideo', 'video/mp4', 'video/mpeg4-generic']
    mattr_reader :content_types, :tempfile_path, :default_processors, :video_types
            ...
            when nil
              ...
              begin
                if processors.any?
                  ...
                  logger.info "attachment_fu trying to load #{attachment_options[:processor]}"
    ...
    module ClassMethods
      ...
      delegate :video_types, :to => Technoweenie::AttachmentFu
      ...
      def video?(content_type)
        # logger.info "Content type recognized as '#{content_type}'"
    ...
    module InstanceMethods
      ...
      def thumbnail_name_for(thumbnail = nil)
        ...
        # only create jpgs for video thumbnails
        ext.sub!(/\.\w+$/, '.jpg') if video?  
      ...
      # Creates or updates the thumbnail for the current attachment.
      def create_or_update_thumbnail_for_movie(temp_file, file_name_suffix, *size)
        returning find_or_initialize_thumbnail(file_name_suffix) do |thumb|
          thumb.attributes = {
      ...
      def uploaded_data=(file_data)
        ...
        if (file_data.content_type == 'unknown/unknown') || (file_data.content_type == 'application/octet-stream')
      ...
      def with_video(&block)
        self.class.with_video(temp_path, &block)
      end
      ...
      protected
        ...
        def after_process_attachment
          if @saved_attachment
            if respond_to?(:process_attachment_with_processing) && thumbnailable? && !attachment_options[:thumbnails].blank? && parent_id.nil? && !video?
              temp_file = temp_path || create_temp_file
              attachment_options[:thumbnails].each { |suffix, size| create_or_update_thumbnail(temp_file, suffix, *size) }
            end
            if respond_to?(:create_video_thumbnails) && video?
        ...
        # def convert_video!(vid,output_format="flv")
        #   convert_video(vid,video_conversion_options)
        # end
