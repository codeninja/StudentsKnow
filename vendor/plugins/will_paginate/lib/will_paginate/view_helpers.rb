...
module WillPaginate
  ...
  module ViewHelpers
    ...
    @@pagination_options = { :class => 'pagination',
          ...
          :param_name   => :page,
          :remote_options => false # specifying link_to_remote options will generate a remote ajax pagination link
          }
  ...
  protected
    ...
    def page_link_or_span(page, span_class, text)
      ...
      unless page
        ...
        unless @options[:remote_options]
          @template.link_to text, @template.params.merge(param => page != 1 ? page : nil)
        else
          @options[:remote_options][:url].merge!(param => page != 1 ? page : nil)
          @template.link_to_remote text,  @options[:remote_options]
        end
      end
    end
